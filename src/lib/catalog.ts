...
export type Product = {
  ...
  price: number;
  /**
   * Путь к фотографии на старом сайте. Сейчас нигде не отрисовывается — в
   * макете на месте фото пустые слоты, ждём оригиналы от заказчика. Пути
   * сохранены, чтобы не пересобирать соответствие «товар → снимок».
   */
  image: string;
  category: CategoryId;
  kind: Kind;
